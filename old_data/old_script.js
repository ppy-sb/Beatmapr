...
let lastLoadedScoreFileContent = ""; // Holds full original content
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // === SEARCH + FETCH PROFILE + TRIGGER SCRIPT ===
  const searchForm = document.querySelector(".search-form");
  const input = searchForm.querySelector("input[name='player']");
  ...
  searchForm.addEventListener("submit", async (event) => {
    ...
    // ✅ Trigger backend to generate score file
  ...
  importFileInput.addEventListener("change", (event) => {
    ...
    reader.onload = function (e) {
      ...
      // Reset beatmap IDs and import new ones
      beatmapIds.clear();
  ...
  // === AUTO-LOAD FILE EVERY 10 SECONDS ===
  async function autoLoadBeatmapFile(filename) {
    try {
      const response = await fetch(filename);
      const content = await response.text();
      lastLoadedScoreFileContent = content; // Save for export
      ...
      // ✅ Show SSH/SH/SS... line if present
